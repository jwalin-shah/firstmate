...
- `fm-verify: OK` / `fm-verify: WARNINGS` - `bin/fm-bootstrap.sh` runs `bin/fm-verify.sh` (config-invariant checks: harness/spawn-template consistency, dispatch-profile and crew/secondmate-harness validity, skill registration, secondmate branch state, required tool presence, backlog/meta consistency) and prints this one-line summary. On `WARNINGS`, run `bin/fm-verify.sh` directly for the full per-check `PASS`/`VIOLATED` report before dispatching new work.
...
- **`bin/` scripts run under macOS's bundled `/bin/bash` (3.2), not just Homebrew bash**, whenever a test restricts `PATH` to system dirs only (e.g. `BASE_PATH=/usr/bin:/bin:/usr/sbin:/sbin`): `#!/usr/bin/env bash` then resolves via that restricted `PATH`. Bash 3.2 has no `declare -A` (associative arrays) at all, and raises `unbound variable` under `set -u` when expanding an empty *indexed* array (`"${arr[@]}"` on `arr=()`) - unlike bash 4.4+, which handles this fine. Avoid both: use plain indexed arrays (never `declare -A`), and never expand a possibly-empty array under `set -u`.
- **A script's own repo-relative sibling files (other `bin/*.sh`) must be located via `SCRIPT_DIR` (derived from `BASH_SOURCE`), never via `FM_ROOT`/`FM_HOME`.** Tests routinely set `FM_ROOT_OVERRIDE` to a bare fixture directory with no `bin/` of its own, to sandbox the *operational home* (`config/`, `state/`, `data/`) being verified - `FM_ROOT`/`FM_HOME` name that operational home, not "where this script's own source lives."
