...
- Only shared material is tracked: `AGENTS.md`, `README.md`, `CONTRIBUTING.md`, `.tasks.toml`, `.github/workflows/`, `bin/`, `.agents/skills/`, and `skills/`.
  ...
  A local `config/backend` file explicitly overrides runtime auto-detection for new task endpoints and stays gitignored; spawn-supported values are `tmux` plus experimental `herdr`.
...
- Changes to runtime session backends (`bin/fm-backend.sh`, `bin/backends/`, and the scripts that dispatch through them) need empirical adapter notes in the relevant backend guide: `docs/tmux-backend.md` or `docs/herdr-backend.md`.
...
tests/fm-watchdog.test.sh                 # fm-watchdog.sh: fresh beacon silence, stale beacon enqueue, missing beacon trigger, and missing wake library survival
...
tests/fm-backend-tmux-edge-cases.test.sh   # edge-case session names for the tmux adapter: numeric, hex, special-char, tmux-keyword, space-containing, and empty session name rejection through the `-t "$ses:"` target-resolution path
