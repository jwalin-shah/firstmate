...
crew_dispatch_validate() {
  ...
  err=$(jq -r '
    def verified($h): ["claude","ct","agy","cursor","codex","opencode","pi","grok"] | index($h);
    def effort_ok($h; $e):
      if $e == null then true
      elif ($e | type) != "string" then false
      elif ($h == "claude" or $h == "ct") then (["low","medium","high","xhigh","max"] | index($e))
      elif ($h == "codex" or $h == "grok" or $h == "pi" or $h == "agy" or $h == "cursor") then (["low","medium","high","xhigh"] | index($e))
