...
make_fake_toolchain() {
  ...
  fm_fake_exit0 "$fakebin" tmux node gh-axi chrome-devtools-axi lavish-axi tldr ccc githits
  cat > "$fakebin/gh" <<'SH'
#!/usr/bin/env bash
...
unverified dispatch harness is flagged^{"rules":[{"when":"anything","use":{"harness":"spaceship"}}],"default":{"harness":"codex"}}^grep^CREW_DISPATCH: invalid config/crew-dispatch.json - unverified harness: spaceship
